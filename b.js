// cpqd.js - Robô CPQD/Telefônica com Menu Interativo (v2 - Página em Branco)
...
const mostrarMenu = async (rl, logado) => {
  limparTela();
  console.log('╔══════════════════════════════════════════════════════════╗');
  ...
  console.log('╠════════════════════════════════════════════════════════ ╣');
  console.log('║  [1] 🔐 Fazer Login no CPQD                            ║');
  console.log('║  [2] 🚀 Fluxo Completo (Facilities → Manobras)         ║');
  console.log('║  [3] 🏢 Abrir apenas Facilities Management             ║');
  console.log('║  [4] 📍 Abrir Site Corrente (CurrentSite)              ║');
  console.log('║  [5] 🔍 Abrir Manobras por CID (manual)                ║');
  console.log('║  [6] 👁️  Modo Inspeção (abre e pausa)                  ║');
  console.log('║  ───────────────────────────────────────────────────── ║');
  console.log('║  [0] 🚪 Sair                                           ║');
  console.log('╚══════════════════════════════════════════════════════════╝');
...
const iniciarNavegador = async () => {
  ...
  // ✅ Abre em página em branco
  const page = await context.newPage();
  await page.goto('about:blank');
  console.log('✅ Navegador iniciado em página em branco!\n');
...
// ============================================================================
// 🔐 OPÇÃO 1: Fazer Login Manualmente
// ============================================================================

const fazerLoginManual = async (page, rl) => {
  console.log('\n🔐 Iniciando login...');
  await fazerLogin(page);
  await aguardarEnter(rl, '\n↩️ Pressione [ENTER] para voltar ao menu...');
  return true; // Retorna logado = true
};

// ============================================================================
// 🏢 OPÇÃO 3: Abrir apenas Facilities
// ============================================================================
...
// ============================================================================
// 🔍 OPÇÃO 5: Abrir Manobras por CID (manual)
// ============================================================================
...
(async () => {
  ...
  let context = null;
  let page = null;
  let logado = false; // ✅ Controla se já está logado
  ...
  try {
    ...
    // ✅ NÃO faz login automático - espera o usuário escolher
    
    while (ativo) {
      try {
        const opcao = await mostrarMenu(rl, logado);
        
        switch (opcao) {
          case '1':
            limparTela();
            console.log('▶️ [1] Fazer Login');
            await fazerLoginManual(page, rl);
            logado = true;
            break;
            
          case '2':
            limparTela();
            console.log('▶️ [2] Fluxo Completo');
            if (!logado) {
              console.log('⚠️ Você precisa fazer login primeiro! (Opção 1)');
              await aguardarEnter(rl, '\n↩️ Pressione [ENTER] para voltar ao menu...');
              break;
            }
          ...
          case '3':
            limparTela();
            console.log('▶️ [3] Abrir Facilities');
            if (!logado) {
              console.log('⚠️ Você precisa fazer login primeiro! (Opção 1)');
              await aguardarEnter(rl, '\n↩️ Pressione [ENTER] para voltar ao menu...');
              break;
          ...
          case '4':
            limparTela();
            console.log('▶️ [4] Abrir Site Corrente');
            if (!logado) {
              console.log('⚠️ Você precisa fazer login primeiro! (Opção 1)');
              await aguardarEnter(rl, '\n↩️ Pressione [ENTER] para voltar ao menu...');
              break;
          ...
          case '5':
            limparTela();
            console.log('▶️ [5] Abrir Manobras por CID');
            if (!logado) {
              console.log('⚠️ Você precisa fazer login primeiro! (Opção 1)');
              await aguardarEnter(rl, '\n↩️ Pressione [ENTER] para voltar ao menu...');
              break;
          ...
          default:
            console.log('\n⚠️ Opção inválida! Escolha de 0 a 6.');
