// login.js - Versão corrigida com tratamento de redirect JSF
        ...
        try {
          ...
          if (urlAtual.includes('layout.xhtml')) {
            ...
            if (encontrarCid && encontrarCid[1]) {
              ...
              // ✅ CORREÇÃO 1: Prepara um listener para capturar nova aba ANTES do goto
              const promessaNovaPagina = context.waitForEvent('page', { timeout: 15000 }).catch(() => null);
              
              // ✅ DIAGNÓSTICO: Mostra quantas abas existem antes do redirect
              console.log(`📊 Abas abertas antes do goto: ${context.pages().length}`);
              context.pages().forEach((p, i) => console.log(`   [${i}] ${p.url()}`));
              
              // Executa o goto
              await novaAba.goto(URLManobrasCorreta, { waitUntil: 'load', timeout: 45000 });
              
              // ✅ CORREÇÃO 2: Aguarda um pouco para o JSF processar o redirect
              await novaAba.waitForTimeout(2000);
              
              // ✅ CORREÇÃO 3: Tenta pegar a nova página que pode ter sido aberta
              const paginaPossivelmenteNova = await promessaNovaPagina;
              
              // ✅ CORREÇÃO 4: Define qual página usar agora
              let paginaManobras = novaAba;
              
              if (paginaPossivelmenteNova) {
                console.log('🔄 Nova aba detectada após o redirect. Alternando referência...');
                try {
                  await paginaPossivelmenteNova.waitForLoadState('load', { timeout: 10000 });
                  paginaManobras = paginaPossivelmenteNova;
                } catch (e) {
                  console.log('⚠️ Nova página não carregou, mantendo referência original.');
                }
              }
              
              // ✅ CORREÇÃO 5: Se ainda não estiver na URL certa, procura nas páginas do contexto
              const urlAtualManobras = paginaManobras.url();
              if (!urlAtualManobras.includes('ManeuverFttxResource')) {
                console.log('🔍 Procurando a página de Manobras entre as abas abertas...');
                const todasPaginas = context.pages();
                const paginaEncontrada = todasPaginas.find(p => p.url().includes('ManeuverFttxResource'));
                
                if (paginaEncontrada) {
                  paginaManobras = paginaEncontrada;
                  console.log(`✅ Página de Manobras encontrada: ${paginaManobras.url()}`);
                } else {
                  console.log('⚠️ Nenhuma página de Manobras encontrada entre as abas.');
                }
              }
              
              // ✅ CORREÇÃO 6: Aguarda estabilização usando a página correta
              console.log('\n⏳ Aguardando estabilização da tela de Manobras...');
              try {
                await paginaManobras.waitForLoadState('networkidle', { timeout: 30000 });
              } catch (e) {
                console.log('⚠️ networkidle não atingido, prosseguindo mesmo assim...');
              }
              
              await paginaManobras.waitForTimeout(3000);
              
              console.log(`🌐 URL final confirmada: ${paginaManobras.url()}`);
              console.log('🚀 Sucesso total! A tela de Manobras está aberta e estável.');
