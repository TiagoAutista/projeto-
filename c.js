...
const {
  ...
  navegarComRetry, extrairDadosTabela, configurarAntiDetecao
  // Removido: selecionarOpcaoDropdown (não usaremos mais)
} = require('./helpers');
...
(async () => {
  ...
  try {
    ...
    // ========================================================================
    // ✅ NOVO FLUXO: Pula o dropdown e vai direto para a pesquisa
    // ========================================================================
    console.log(`\n⌨️ Digitando o ID diretamente na barra de pesquisa: ${CONFIG.idFibra}`);
    
    // Aguarda o campo de pesquisa ficar visível e interativo
    await page.waitForSelector(CONFIG.selectors.searchInput, { visible: true, timeout: CONFIG.timeouts.element });
    
    // Clica no campo, limpa qualquer valor residual e digita o ID
    ...
    console.log("✅ ID digitado com sucesso!\n");
    ...
    if (dadosExtraidos.length > 0) {
      console.log("📋 Prévia:", JSON.stringify(dadosExtraidos[0], null, 2));
    }
    ...
    try {
      ...
      console.error("⚠️ Falha ao salvar screenshot de erro:", e.message); 
    }
  } finally {
    console.log("\n🔒 Fechando navegador...");
    await browser.close();
