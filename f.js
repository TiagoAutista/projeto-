// src/main.js (ou index.js - seu orquestrador principal)
...
(async () => {
  ...
  try {
    ...
    console.error("❌ Erro fatal ao conectar ao Chrome (CDP):", err.message);
    console.error("💡 Verifique se o Chrome foi iniciado com flags de depuração.");
    rl.close();
    process.exit(1);
  }
  ...
  while (ativo) {
    try {
      ...
      switch (op) {
        ...
        case "9":
          console.clear();
          console.log('▶️ [SDU] Iniciando busca de ID Fibra...');
        ...
        // ✅ CASO 0 (encerrar) - mais intuitivo que 9
        case "0":
        case "99":
        ...
        default:
          console.log("⚠️ Opção inválida! Escolha um número válido.");
          await new Promise(r => setTimeout(r, 1500));
      }
    } catch (err) {
      console.error("\n❌ Erro na execução:", err.message);
      console.log("💡 O estado do robô foi preservado.");
      await aguardarVoltar();
    }
  }
...
process.on('unhandledRejection', (erro) => {
  console.error('\n❌ Erro assíncrono inesperado:', erro.message);
});
