...
const menuCPQD = async (rl) => {
  ...
  while (ativo) {
    ...
    console.log('║  [2] 🚀 Fluxo Completo                                 ║');
    console.log('║  [3] 🏢 Abrir Facilities                               ║');
    console.log('║  [4] 📍 Abrir Site Corrente                            ║');
    console.log('║  [5] 🔍 Abrir Manobras por CID                         ║');
    console.log('║  [6] 👁️  Modo Inspeção                                 ║');
    console.log('║  [7] ❌ Fechar Navegador CPQD                          ║');
    ...
    try {
      switch (opcao) {
        ...
        case '2': if (!logado) { console.log('⚠️ Faça login primeiro!'); await aguardarEnter(rl); break; } await fluxoCompleto(rl); break;
        case '3': if (!logado) { console.log('⚠️ Faça login primeiro!'); await aguardarEnter(rl); break; } await abrirFacilities(rl); break;
        case '4': if (!logado) { console.log('⚠️ Faça login primeiro!'); await aguardarEnter(rl); break; } await abrirSiteCorrente(rl); break;
        case '5': if (!logado) { console.log('⚠️ Faça login primeiro!'); await aguardarEnter(rl); break; } await abrirManobrasPorCID(rl); break;
