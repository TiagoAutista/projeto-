...
(async () => {
  ...
  try {
    ...
    await page.waitForSelector('#mat-select-0', { visible: true, timeout: 15000 });
    ...
    await page.waitForSelector('.mat-option', { visible: true, timeout: 10000 });
    ...
    await page.evaluate(() => {
      ...
      const opcaoAlvo = opcoes.find(opt => opt.innerText.trim().includes('ID Fibra'));
      if (opcaoAlvo) {
        opcaoAlvo.click();
      } else {
        console.warn('⚠️ Opção "ID Fibra" não encontrada. Verifique o nome exato no sistema.');
      }
    });
    ...
    await page.$eval('input[formcontrolname="search"]', el => el.value = ''); 
    ...
    await page.waitForSelector('.ui-home-state, .mat-table, .ui-card', { timeout: 15000 });
    
    console.log('✅ Busca realizada com sucesso! O robó pode prosseguir com a extração de dados.');
