...
// ✅ NOVO: Módulo Diagnóstico Unificado
const { processarDiagnosticoUnificado, abrirParaInspecaoDiagnostico } = require("./lib/diagnostico-unificado");
  ...
  while (ativo) {
    try {
      ...
      switch (op) {
        ...
        // ✅ NOVAS OPÇÕES: Sistema de Diagnóstico Unificado
        case "9":
          console.clear();
          console.log("🔬 [Diagnóstico Unificado] Iniciando processamento em lote...");
          page = await obterPagina(browser, "diagnostico");
          await configurarPagina(page, config);
          await garantirSessao(page, "diagnostico", rl, config);
          await processarDiagnosticoUnificado(page, "lote", config, rl);
          await aguardarVoltar();
          break;

        case "10":
          console.clear();
          console.log("🔍 [Diagnóstico Unificado] Abrindo para inspeção/ajustes manuais...");
          page = await obterPagina(browser, "diagnostico");
          await configurarPagina(page, config);
          await garantirSessao(page, "diagnostico", rl, config);
          await abrirParaInspecaoDiagnostico(page, config);
          await aguardarVoltar();
          break;
        ...
        default:
          console.log("⚠️ Opção inválida! Escolha um número de 1 a 11.");
